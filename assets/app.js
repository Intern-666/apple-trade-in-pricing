...
let configData = {};
let modelNumberData = {};
const customerDetails = JSON.parse(sessionStorage.getItem("customerDetails") || "null");
if (!customerDetails) {
  window.location.href = "/customer-detail";
}
const DEVICE_IMAGE_MAP = { "iphone": { "standard": "iphone/standard.png", "plus": "iphone/plus.png", "pro": "iphone/pro.png", "pro-max": "iphone/pro-max.png", "mini": "iphone/mini.png", "se": "iphone/se.png", "air": "iphone/air.png", "default": "iphone/base.png" }, "ipad": { "ipad": "ipad/ipad.png", "ipad-air": "ipad/air.png", "ipad-pro": "ipad/pro.png", "ipad-mini": "ipad/mini.png", "default": "ipad/base.png" }, "mac": { "macbook": "mac/macbook.png", "macbook-air": "mac/macbook-air.png", "macbook-pro": "mac/macbook-pro.png", "imac": "mac/imac.png", "imac-pro": "mac/imac-pro.png", "mac-mini": "mac/mac-mini.png", "mac-studio": "mac/mac-studio.png", "mac-pro": "mac/mac-pro.png", "default": "mac/base.png" }, "apple-watch": { "watch-se": "watch/se.png", "apple-watch-se": "watch/se.png", "se": "watch/se.png", "watch-series": "watch/series.png", "apple-watch-series": "watch/series.png", "series": "watch/series.png", "watch-ultra": "watch/ultra.png", "apple-watch-ultra": "watch/ultra.png", "ultra": "watch/ultra.png", "default": "watch/base.png" }, "airpods": { "airpods": "airpods/standard.png", "airpods-pro": "airpods/pro.png", "airpods-max": "airpods/max.png", "default": "airpods/base.png" } };
...
function resolveDeviceImage(device, subDevice) {
  ...
  const aliasKey = key.replace(/^apple-watch-/, "").replace(/^watch-/, "");
  const image = deviceMap[key] || deviceMap[aliasKey] || deviceMap["default"] || null;
  return image;
}
function isMacDesktopSubDevice(subDevice) {
  const value = String(subDevice || "").toLowerCase();
  return value.includes("mini") || value.includes("studio") || value.includes("pro desktop") || value.includes("mac pro") || value.includes("mac-pro") || value.includes("pro") && !value.includes("macbook");
}
function updateDeviceImagePreview(device, subDevice) {
  ...
  const relativeSrc = resolveDeviceImage(device, subDevice);
  img.src = relativeSrc ? `/assets/devices/${relativeSrc}` : DEVICE_IMAGE_FALLBACK;
...
document.addEventListener("click", function(e) {
  ...
  if (modelNumberHelpBtn && modelNumberHelp && !modelNumberHelpBtn.contains(e.target) && !modelNumberHelp.contains(e.target)) {
    modelNumberHelp.classList.add("hidden");
    modelNumberHelpBtn.setAttribute("aria-expanded", "false");
  }
});
const modelNumberHelpData = { iPhone: { label: "Where can I find my iPhone model number?", url: "https://support.apple.com/en-my/106343" }, iPad: { label: "Where can I find my iPad model number?", url: "https://support.apple.com/en-my/106343" }, "Apple Watch": { label: "Where can I find my Apple Watch model number?", url: "https://support.apple.com/en-my/108056" }, AirPods: { label: "Where can I find my AirPods model number?", url: "https://support.apple.com/en-my/109525" }, Mac: { label: "Where can I find my Mac model identifier?", url: "https://support.apple.com/en-my/102767" } };
function updateModelNumberHelp() {
  if (!modelNumberHelpBtn || !modelNumberHelp || !modelNumberHelpText || !modelNumberHelpLink) {
    return;
  }
  const selectedDevice = deviceSelect.value;
  const helpData = modelNumberHelpData[selectedDevice];
  if (!helpData) {
  ...
  modelNumberHelpText.textContent = helpData.label;
  modelNumberHelpLink.href = helpData.url;
}
if (modelNumberHelpBtn) {
  modelNumberHelpBtn.addEventListener("click", function() {
    const isHidden = modelNumberHelp.classList.toggle("hidden");
    modelNumberHelpBtn.setAttribute("aria-expanded", String(!isHidden));
  });
}
let currentStep = 1;
...
function getStepIndicatorItem(step) {
  return stepIndicator.querySelector(`.step-indicator-item[data-step="${step}"]`);
}
function goToStep(targetStep, direction) {
  ...
  if (outgoing && outgoing !== incoming) {
    outgoing.classList.remove("step-panel-active", "step-panel-enter-right", "step-panel-enter-left");
  }
  incoming.classList.remove("step-panel-enter-right", "step-panel-enter-left");
  incoming.classList.add("step-panel-active");
  void incoming.offsetWidth;
  incoming.classList.add(direction === "back" ? "step-panel-enter-left" : "step-panel-enter-right");
...
function resetFromStep(step) {
  ...
  if (step < 4) {
    storageTypeSelect.innerHTML = '<option value="" disabled selected>Select storage type...</option>';
    connectivitySelect.innerHTML = '<option value="" disabled selected>Select connectivity...</option>';
...
function refreshStepIndicator() {
  for (let step = 1; step <= 5; step++) {
    ...
    const isUnlocked = step <= maxUnlockedStep && (step !== 4 || step4Applicable);
    const isCurrent = step === currentStep;
    const isCompleted = isUnlocked && step < currentStep;
    item.classList.toggle("unlocked", isUnlocked);
    item.classList.toggle("current", isCurrent);
    item.classList.toggle("completed", isCompleted);
    const button = item.querySelector(".step-indicator-button");
    if (button) {
...
stepIndicator.addEventListener("click", function(event) {
  const button = event.target.closest("[data-goto-step]");
  if (!button || button.disabled) {
    return;
  }
  const targetStep = parseInt(button.dataset.gotoStep, 10);
  ...
  goToStep(targetStep, targetStep < currentStep ? "back" : "forward");
});
window.addEventListener("DOMContentLoaded", async function() {
  const customerDetails2 = sessionStorage.getItem("customerDetails");
  if (!customerDetails2) {
    window.location.replace("/customer-detail");
    return;
  }
  try {
    const [modelsResponse, configResponse, numbersResponse] = await Promise.all([fetch("/available-models"), fetch("/model-configuration"), fetch("/model-numbers")]);
    if (!modelsResponse.ok) {
      throw new Error(`Failed to load models: ${modelsResponse.status}`);
    }
    deviceData = await modelsResponse.json();
    configData = configResponse.ok ? await configResponse.json() : {};
    ...
    for (const device in deviceData) {
      ...
      deviceBoxGrid.appendChild(buildDeviceBox(device));
    }
    ...
    deviceSelect.innerHTML = `<option value="" disabled selected>We couldn't load the device list</option>`;
    deviceBoxGrid.innerHTML = `<p class="field-help">We couldn't load the device list. Please refresh the page and try again.</p>`;
    getStepPanel(1).classList.add("step-panel-active");
    refreshStepIndicator();
  }
});
function buildDeviceBox(device) {
  ...
  if (imageSrc) {
    ...
    img.addEventListener("error", function() {
      img.replaceWith(buildIconFallback(device));
    });
    box.appendChild(img);
  } else {
    box.appendChild(buildIconFallback(device));
  }
...
function selectDeviceBox(device) {
  ...
  deviceBoxGrid.querySelectorAll(".device-box").forEach((box) => {
    const isSelected = box.dataset.device === device;
...
deviceSelect.addEventListener("change", function(event) {
  ...
  for (const sub in subDevices) {
    ...
    subDeviceBoxGrid.appendChild(buildSubDeviceBox(selectedDevice, sub));
  }
  subDeviceSelect.disabled = false;
  subDeviceBoxGrid.classList.remove("disabled");
  modelSelect.innerHTML = '<option value="" disabled selected>Select a model...</option>';
  modelSelect.disabled = true;
  storageSelect.required = selectedDevice !== "AirPods";
...
subDeviceSelect.addEventListener("change", function(event) {
  ...
  resetFromStep(2);
  unlockStep(3);
  const subDeviceNumbers = modelNumberData[selectedDevice] && modelNumberData[selectedDevice][selectedSub] ? modelNumberData[selectedDevice][selectedSub] : {};
...
function buildSubDeviceBox(device, sub) {
  ...
  if (imageSrc) {
    ...
    img.addEventListener("error", function() {
      img.replaceWith(buildSubDeviceIconFallback(sub));
    });
    box.appendChild(img);
  } else {
    box.appendChild(buildSubDeviceIconFallback(sub));
  }
  const label = document.createElement("span");
...
function selectSubDeviceBox(sub) {
  ...
  subDeviceBoxGrid.querySelectorAll(".subdevice-box").forEach((box) => {
    const isSelected = box.dataset.sub === sub;
    box.classList.toggle("selected", isSelected);
    box.setAttribute("aria-selected", isSelected ? "true" : "false");
  });
  subDeviceSelect.dispatchEvent(new Event("change"));
}
modelSelect.addEventListener("change", function(event) {
  ...
  const modelConfig = configData[selectedDevice] && configData[selectedDevice][selectedSub] && configData[selectedDevice][selectedSub][selectedModel] || { storageTypes: [], connectivity: [] };
  storageTypeSelect.innerHTML = '<option value="" disabled selected>Select storage type...</option>';
  ...
  step4Applicable = hasStorage || hasStorageType || hasConnectivity;
  resultCard.classList.add("hidden");
  tradeInCard.classList.remove("result-shown");
  if (step4Applicable) {
    unlockStep(4);
...
function checkConfigurationComplete() {
  const selectedDevice = deviceSelect.value;
  const selectedSub = subDeviceSelect.value;
  const storageOk = storageGroup.classList.contains("hidden") || storageSelect.value !== "";
  const storageTypeOk = storageTypeGroup.classList.contains("hidden") || storageTypeSelect.value !== "";
  const connectivityOk = connectivityGroup.classList.contains("hidden") || connectivitySelect.value !== "";
...
function showConditionProfile(selectedDevice, selectedSub) {
  ...
  if (selectedDevice === "iPhone") {
    ...
    if (isMacDesktopSubDevice(subLower)) {
      ...
      showProfile("profileMacDesktop");
      const screenGroup = document.getElementById("desktopScreenGroup");
      if (screenGroup) {
...
function hideConditionProfiles() {
  const profiles = ["profileIPhone", "profileIPad", "profileAppleWatch", "profileMacLaptop", "profileMacDesktop", "profileAirPods"];
  profiles.forEach((id) => {
    const profile = document.getElementById(id);
    if (profile) {
...
async function fetchPredictedPrice(device, subDevice, model, storage, storageType, connectivity) {
  const apiUrl = "/predict";
  try {
    const payload = { Device: device, SubDevice: subDevice, Model: model, Storage: storage ? parseFloat(storage) : null, StorageType: storageType || null, Connectivity: connectivity || null };
...
function validateConditionSelections() {
  ...
  let requiredFields = [];
  if (selectedDevice === "iPhone") {
    requiredFields = [["iphoneScreen", "screen condition"], ["iphoneBody", "body & frame condition"], ["iphoneBattery", "battery health"]];
  } else if (selectedDevice === "iPad") {
    requiredFields = [["ipadScreen", "screen condition"], ["ipadBody", "body condition"], ["ipadBattery", "battery health"]];
  } else if (selectedDevice === "Apple Watch") {
    requiredFields = [["watchScreen", "screen condition"], ["watchBody", "case condition"], ["watchBattery", "battery health"]];
  } else if (selectedDevice === "AirPods") {
    requiredFields = [["airpodsCase", "charging case condition"], ["airpodsBuds", "earbuds condition"], ["airpodsBattery", "battery condition"]];
  } else if (selectedDevice === "Mac") {
    const subLower = (selectedSub || "").toLowerCase();
    if (isMacDesktopSubDevice(subLower)) {
      requiredFields = [["desktopBody", "body condition"]];
    } else if (subLower.includes("imac")) {
      requiredFields = [["desktopScreen", "screen condition"], ["desktopBody", "body condition"]];
    } else {
      requiredFields = [["laptopScreen", "screen condition"], ["laptopBody", "body condition"], ["laptopBattery", "battery health"]];
    }
  }
  requiredFields.forEach(([id]) => {
    clearConditionFieldError(document.getElementById(id));
  });
  ...
  missingFields.forEach(([id, label]) => {
    const field = document.getElementById(id);
    if (field) {
      showConditionFieldError(field, `Please select your ${label}.`);
    }
  });
  const firstMissingField = document.getElementById(missingFields[0][0]);
...
tradeInForm.addEventListener("submit", async function(event) {
  event.preventDefault();
  ...
  const valuation = await fetchPredictedPrice(device, subDevice, model, storage, storageType, connectivity);
  submitBtn.disabled = false;
  if (buttonText) {
    buttonText.textContent = "Get my valuation";
  }
  if (buttonArrow) {
    buttonArrow.textContent = "\u2192";
  }
  if (valuation === null) {
    resultValue.innerText = "We couldn't calculate an estimate";
    return;
  }
  if (valuation.status !== "resolved" || valuation.estimated_value === null) {
    resultValue.innerHTML = `
          <div class="no-pricing-message">
              <div class="no-pricing-title">
                  Pricing unavailable
              </div>

              <div class="no-pricing-text">
                  We don't have pricing for this exact configuration yet.
                  Please try a different option or check back later.
              </div>
          </div>
      `;
    return;
  }
  const medianPrice = Number(valuation.estimated_value);
  let score = 100;
  ...
  if (profileIPhone && !profileIPhone.classList.contains("hidden")) {
    score += parseInt(document.getElementById("iphoneScreen").value);
    score += parseInt(document.getElementById("iphoneBody").value);
    score += parseInt(document.getElementById("iphoneBattery").value);
    document.querySelectorAll('input[name="iphoneDefect"]:checked').forEach((cb) => {
      score += parseInt(cb.value);
    });
  } else if (profileIPad && !profileIPad.classList.contains("hidden")) {
    score += parseInt(document.getElementById("ipadScreen").value);
    score += parseInt(document.getElementById("ipadBody").value);
    score += parseInt(document.getElementById("ipadBattery").value);
    document.querySelectorAll('input[name="ipadDefect"]:checked').forEach((cb) => {
      score += parseInt(cb.value);
    });
  } else if (profileAppleWatch && !profileAppleWatch.classList.contains("hidden")) {
    score += parseInt(document.getElementById("watchScreen").value);
    score += parseInt(document.getElementById("watchBody").value);
    score += parseInt(document.getElementById("watchBattery").value);
    document.querySelectorAll('input[name="watchDefect"]:checked').forEach((cb) => {
      score += parseInt(cb.value);
    });
  } else if (profileMacLaptop && !profileMacLaptop.classList.contains("hidden")) {
    score += parseInt(document.getElementById("laptopScreen").value);
    score += parseInt(document.getElementById("laptopBody").value);
    score += parseInt(document.getElementById("laptopBattery").value);
    document.querySelectorAll('input[name="laptopDefect"]:checked').forEach((cb) => {
      score += parseInt(cb.value);
    });
  } else if (profileMacDesktop && !profileMacDesktop.classList.contains("hidden")) {
    const selectedSub = subDeviceSelect.value;
    if (selectedSub && selectedSub.toLowerCase().includes("imac")) {
      const desktopScreen = document.getElementById("desktopScreen");
      if (desktopScreen) {
        score += parseInt(desktopScreen.value);
      }
    }
    score += parseInt(document.getElementById("desktopBody").value);
    document.querySelectorAll('input[name="desktopDefect"]:checked').forEach((cb) => {
      score += parseInt(cb.value);
    });
  } else if (profileAirPods && !profileAirPods.classList.contains("hidden")) {
    score += parseInt(document.getElementById("airpodsCase").value);
    score += parseInt(document.getElementById("airpodsBuds").value);
    score += parseInt(document.getElementById("airpodsBattery").value);
    document.querySelectorAll('input[name="airpodsDefect"]:checked').forEach((cb) => {
      score += parseInt(cb.value);
    });
  }
  ...
  const finalPrice = Math.floor(medianPrice * multiplier);
  const formattedPrice = finalPrice.toLocaleString("en-MY");
  const tradeInRecord = { customer: customerDetails, device: { device, subDevice, modelNumber: modelNumberInput.value.trim() || null, model, storage: storage || null, storageType: storageType || null, connectivity: connectivity || null }, valuation: { marketValue: medianPrice, conditionScore: score, grade, multiplier, finalValue: finalPrice }, createdAt: (/* @__PURE__ */ new Date()).toISOString() };
  sessionStorage.setItem("tradeInRecord", JSON.stringify(tradeInRecord));
  try {
    const saveResponse = await fetch("/customer/trade-in", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(tradeInRecord) });
    const saveResult = await saveResponse.json();
    if (!saveResponse.ok || saveResult.status !== "success") {
      console.error("Failed to save trade-in record:", saveResult);
    }
  } catch (error) {
    console.error("Customer trade-in save error:", error);
  }
  resultValue.innerHTML = `RM ${formattedPrice}`;
  resultCard.scrollIntoView({ behavior: "smooth", block: "center" });
});
startOverBtn.addEventListener("click", function() {
  ...
  deviceBoxGrid.querySelectorAll(".device-box.selected").forEach((box) => {
