import re; import difflib; import numpy as np; import pandas as pd; CANONICAL_FIELDS = ['Provider', 'Device', 'Sub-device', 'Model Number', 'Standardized Model', 'Retail Price', 'Storage (GB)', 'Storage Type', 'Connectivity', 'Material', 'Max. Trade-In Value (RM)', 'Model_Year', 'Case Size', 'Charging Method', 'Collection Date']; CONFIGURATION_FIELDS = ['Device', 'Sub-device', 'Standardized Model', 'Model Number', 'Storage (GB)', 'Storage Type', 'Connectivity', 'Material', 'Model_Year', 'Case Size', 'Charging Method']; DUPLICATE_FIELDS = [f for f in CANONICAL_FIELDS if f != 'Collection Date']; NUMERIC_FIELDS = {'Retail Price', 'Storage (GB)', 'Max. Trade-In Value (RM)', 'Model_Year', 'Case Size'}; OPTIONAL_FIELDS = {'Provider', 'Model Number', 'Storage Type', 'Connectivity', 'Material', 'Max. Trade-In Value (RM)', 'Case Size', 'Charging Method'}; DEVICE_APPLICABLE_FIELDS = {'iPhone': {'Storage (GB)', 'Model_Year'}, 'iPad': {'Storage (GB)', 'Connectivity', 'Model_Year'}, 'Mac': {'Storage (GB)', 'Storage Type', 'Model_Year'}, 'Apple Watch': {'Storage (GB)', 'Connectivity', 'Material', 'Case Size', 'Model_Year'}, 'AirPods': {'Charging Method', 'Model_Year'}}
def is_field_applicable(device, field):
    if field in {'Provider', 'Device', 'Sub-device', 'Model Number', 'Standardized Model', 'Retail Price', 'Max. Trade-In Value (RM)'}:
        return True
...
def normalize_text(value):
    if _is_blank(value):
        return None
    text = str(value).strip().lower(); text = text.replace('&', ' and '); text = re.sub('[^a-z0-9]+', ' ', text); text = re.sub('\\s+', ' ', text).strip(); return text or None
def strip_brand_words(text):
    if not text:
        return ''
    tokens = [t for t in str(text).split() if t.lower() not in BRAND_PREFIX_WORDS]; return ' '.join(tokens)
def normalize_device_value(value):
    if _is_blank(value):
        return None
    text = normalize_text(value); return DEVICE_ALIASES.get(text) if text else None
def json_safe(value):
...
def build_device_vocabulary(master_df):
    ...
    for _, row in master_df.iterrows():
        device = normalize_device_value(row.get('Device')); sub_device = normalize_text(row.get('Sub-device'))
        if device:
            devices.add(device)
        if sub_device:
...
def infer_model_year_from_text(value):
    if _is_blank(value):
        return None
    match = re.search('\\b(20\\d{2})\\b', str(value)); return int(match.group(1)) if match else None
def infer_device_subdevice(row_text, vocabulary):
    if not row_text:
        return (None, None)
    normalized = normalize_text(row_text) or ''; device_matches = []
    for device in vocabulary.get('devices', set()):
        d_text = normalize_text(device)
        if d_text and re.search(f'\\b{re.escape(d_text)}\\b', normalized):
            device_matches.append(device)
    for alias, canonical in DEVICE_ALIASES.items():
        if canonical in vocabulary.get('devices', set()) and re.search(f'\\b{re.escape(alias)}\\b', normalized):
    ...
    if len(device_matches) != 1:
        return (None, None)
    device = device_matches[0]; sub_matches = []
    for sub_device in vocabulary.get('sub_devices', set()):
        s_text = normalize_text(sub_device)
        if s_text and re.search(f'\\b{re.escape(s_text)}\\b', normalized):
            sub_matches.append(sub_device)
    sub_matches = sorted(set(sub_matches))
    if len(sub_matches) == 1:
        return (device, sub_matches[0])
    if len(sub_matches) > 1:
        word_sets = {c: set(normalize_text(c).split()) for c in sub_matches}; most_specific = [c for c, w in word_sets.items() if not any((o != c and w < word_sets[o] for o in sub_matches))]
...
def _parse_numeric_value(value, field_name=None):
    ...
    if field_name == 'Storage (GB)':
        match = re.fullmatch('(?i)\\s*(\\d+(?:\\.\\d+)?)\\s*(gb|tb)?\\s*', text)
        if not match:
            return (None, True)
...
def _pretokenize_storage_text(value):
    if _is_blank(value):
        return value
    return re.sub('(?i)(\\d)(gb|tb)\\b', '\\1 \\2', str(value))
_MAC_MODEL_IDENTIFIER_RE = re.compile('[A-Za-z]+\\d+,\\d+')
def _split_model_number_text(text, is_mac):
    if not is_mac:
        return re.split('[,/;|]+', text)
    protected = []
    def _protect(match):
        p = f'\x00{len(protected)}\x00'; protected.append(match.group(0)); return p
    protected_text = _MAC_MODEL_IDENTIFIER_RE.sub(_protect, text); raw_parts = re.split('[,/;|]+', protected_text); restored = []
...
def normalize_model_numbers(value, device=None):
    if _is_blank(value):
        return frozenset()
    is_mac = normalize_device_value(device) == 'Mac'; parts = _split_model_number_text(str(value), is_mac); normalized = set()
    for part in parts:
        token = re.sub('[^A-Za-z0-9,]' if is_mac and _MAC_MODEL_IDENTIFIER_RE.search(part) else '[^A-Za-z0-9]', '', part).upper()
        if token:
            normalized.add(token)
    return frozenset(normalized)
_MAC_IDENTIFIER_FAMILY_CASING = {'MAC': 'Mac', 'MACBOOK': 'MacBook', 'MACBOOKAIR': 'MacBookAir', 'MACBOOKPRO': 'MacBookPro', 'MACMINI': 'Macmini', 'MACPRO': 'MacPro', 'MACSTUDIO': 'MacStudio', 'IMAC': 'iMac', 'IMACPRO': 'iMacPro'}; _MAC_IDENTIFIER_LETTERS_RE = re.compile('^([A-Za-z]+)(\\d+,\\d+)$')
...
def normalize_master_model_number_column(df):
    if df is None or df.empty or 'Model Number' not in df.columns:
        return df
    result = df.copy(); has_device = 'Device' in result.columns
...
def infer_subdevice_from_model_numbers(tokens, mapping):
    if not tokens or not mapping:
        return None
    matched = {mapping.get(t) for t in tokens if mapping.get(t) is not None}; return next(iter(matched)) if len(matched) == 1 else None
def normalize_connectivity_value(value):
    if _is_blank(value):
        return None
    text = normalize_text(value) or ''; has_cel = any((p in text for p in ('cellular', '5g', '4g', 'lte', 'sim'))); has_wifi = any((p in text for p in ('wi fi', 'wifi', 'wi-fi')))
...
def extract_model_connectivity(model_text):
    if not model_text:
        return ('', None)
    text = normalize_text(model_text) or ''; has_cel = any((p in text for p in ('cellular', '5g', '4g', 'lte', 'sim'))); has_wifi = any((p in text for p in ('wi fi', 'wifi', 'wi-fi'))); conn = 'wi fi cellular' if has_wifi and has_cel else 'cellular' if has_cel else 'wi fi' if has_wifi else None; cleaned = re.sub('\\bwi[\\s-]?fi\\b|\\bwi-fi\\b|\\bcellular\\b|\\b5g\\b|\\b4g\\b|\\blte\\b|\\bsim\\b', ' ', text, flags=re.IGNORECASE); return (re.sub('\\s+', ' ', cleaned).strip(), conn)
...
def canonicalize_incoming_row_display_values(row, display_maps):
    updates = {}; raw_dev = row.get('Device'); c_dev = normalize_device_value(raw_dev)
    if c_dev and c_dev != raw_dev:
        updates['Device'] = c_dev
    dev_key = c_dev or normalize_device_value(raw_dev); raw_sub = row.get('Sub-device')
    ...
    if not _is_blank(raw_model):
        stripped, embedded_conn = extract_model_connectivity(strip_brand_words(normalize_text(raw_model))); c_model = display_maps['model'].get((dev_key, stripped))
...
def extract_apple_model_identity(model_text):
    ...
    text = normalize_text(model_text) or ''; tokens = text.split(); product_line = next((c for c in ['iphone', 'ipad', 'macbook', 'imac', 'mac mini', 'mac studio', 'mac pro', 'apple watch', 'airpods'] if c in text), None); variants, generations, designations, screen_sizes = (set(), set(), set(), set())
    for token in tokens:
    ...
    for m in re.finditer('\\b(\\d+(?:\\.\\d+)?)\\s*[- ]?inch\\b', text):
...
def _designation_numeric_bases(designations):
    return {re.match('(\\d+)', d).group(1) for d in designations if re.match('(\\d+)', d)}
def model_identity_conflict(incoming_model, master_model):
    ...
    if inc['screen_sizes'] and mst['screen_sizes'] and inc['screen_sizes'].isdisjoint(mst['screen_sizes']):
        return True
    inc_bases = inc['generations'] | _designation_numeric_bases(inc['model_designations']); mst_bases = mst['generations'] | _designation_numeric_bases(mst['model_designations'])
    if inc_bases and mst_bases and inc_bases.isdisjoint(mst_bases):
        return True
    return False
def propose_column_mapping(raw_columns):
    norm_raw = {c: normalize_text(c) or '' for c in raw_columns}; norm_synonyms = {c: {normalize_text(a) for a in aliases} for c, aliases in COLUMN_SYNONYMS.items()}; mapping, used = ({}, set())
    for canonical in CANONICAL_FIELDS:
        cands = norm_synonyms.get(canonical, set()); cands.add(normalize_text(canonical))
        for raw, norm_r in norm_raw.items():
            if raw not in used and norm_r in cands:
                mapping[raw] = canonical; used.add(raw); break
    for canonical in CANONICAL_FIELDS:
        if canonical in mapping.values():
            continue
        cands = norm_synonyms.get(canonical, set()); cands.add(normalize_text(canonical)); best_raw, best_score = (None, 0)
        for raw, norm_r in norm_raw.items():
        ...
        if best_raw and best_score >= 60:
            mapping[best_raw] = canonical; used.add(best_raw)
...
def apply_column_mapping(raw_df, mapping):
    canonical_df = pd.DataFrame(index=raw_df.index); rev = {v: k for k, v in mapping.items()}
    for canonical in CANONICAL_FIELDS:
...
def normalize_row_for_matching(row, mapped_fields=None):
    mapped_fields = set(mapped_fields or CANONICAL_FIELDS); raw_dev = _get_mapped_value(row, 'Device', mapped_fields); raw_model = _get_mapped_value(row, 'Standardized Model', mapped_fields); device = normalize_device_value(raw_dev); model_text, embedded_conn = extract_model_connectivity(strip_brand_words(normalize_text(raw_model))); explicit_conn = normalize_connectivity_value(_get_mapped_value(row, 'Connectivity', mapped_fields)); return {'device': device or UNKNOWN, 'sub_device': normalize_text(_get_mapped_value(row, 'Sub-device', mapped_fields)) or UNKNOWN, 'model_text': model_text or UNKNOWN, 'storage': normalize_numeric_for_comparison(_get_mapped_value(row, 'Storage (GB)', mapped_fields), 'Storage (GB)'), 'storage_type': normalize_text(_get_mapped_value(row, 'Storage Type', mapped_fields)) or UNKNOWN, 'connectivity': explicit_conn if explicit_conn is not None else embedded_conn or UNKNOWN, 'material': normalize_text(_get_mapped_value(row, 'Material', mapped_fields)) or UNKNOWN, 'model_year': normalize_numeric_for_comparison(_get_mapped_value(row, 'Model_Year', mapped_fields), 'Model_Year'), 'provider': normalize_text(_get_mapped_value(row, 'Provider', mapped_fields)) or UNKNOWN, 'case_size': normalize_numeric_for_comparison(_get_mapped_value(row, 'Case Size', mapped_fields), 'Case Size'), 'charging_method': normalize_text(_get_mapped_value(row, 'Charging Method', mapped_fields)) or UNKNOWN, 'retail_price': normalize_numeric_for_comparison(_get_mapped_value(row, 'Retail Price', mapped_fields), 'Retail Price'), 'trade_in_value': normalize_numeric_for_comparison(_get_mapped_value(row, 'Max. Trade-In Value (RM)', mapped_fields), 'Max. Trade-In Value (RM)'), 'model_numbers': normalize_model_numbers(_get_mapped_value(row, 'Model Number', mapped_fields), device=raw_dev)}
...
def validate_new_row_fields(device, sub_device, model_name, msrp, trade_in_value, storage, storage_type, connectivity, material, case_size, charging_method, model_year):
    ...
    elif _parse_numeric_value(msrp, 'Retail Price')[1]:
        errors.append('Retail Price must be numeric.')
    elif _parse_numeric_value(msrp, 'Retail Price')[0] < 0:
        errors.append('Retail Price cannot be negative.')
    return errors
def _field_value(norm, field):
    mapping = {'Device': 'device', 'Sub-device': 'sub_device', 'Standardized Model': 'model_text', 'Storage (GB)': 'storage', 'Storage Type': 'storage_type', 'Connectivity': 'connectivity', 'Material': 'material', 'Model_Year': 'model_year', 'Case Size': 'case_size', 'Charging Method': 'charging_method', 'Provider': 'provider', 'Retail Price': 'retail_price', 'Max. Trade-In Value (RM)': 'trade_in_value'}; return norm.get(mapping[field])
def extract_mac_configuration(model_text):
    ...
    text = normalize_text(model_text) or ''; return {'chip': (re.search('\\bm[1-4](?:\\s+(?:pro|max|ultra))?\\b', text) or [None])[0] and re.search('\\bm[1-4](?:\\s+(?:pro|max|ultra))?\\b', text).group(0), 'cpu_tier': (re.search('\\bi[3579]\\b', text) or [None])[0] and re.search('\\bi[3579]\\b', text).group(0), 'cpu_frequency': float(re.search('\\b(\\d+(?:\\.\\d+)?)\\s*ghz\\b', text).group(1)) if re.search('\\b(\\d+(?:\\.\\d+)?)\\s*ghz\\b', text) else None, 'cpu_cores': int(re.search('\\b(\\d+)\\s*core\\s+cpu\\b', text).group(1)) if re.search('\\b(\\d+)\\s*core\\s+cpu\\b', text) else None, 'gpu_cores': int(re.search('\\b(\\d+)\\s*core\\s+gpu\\b', text).group(1)) if re.search('\\b(\\d+)\\s*core\\s+gpu\\b', text) else None}
def _mac_configuration_match_score(incoming_model, master_model):
    inc, mst = (extract_mac_configuration(incoming_model), extract_mac_configuration(master_model)); score, weight = (0.0, 0.0)
...
def _model_similarity(incoming_model, master_model):
    ...
    if model_identity_conflict(inc, mst):
        return 0.0
    seq_score = difflib.SequenceMatcher(None, inc, mst).ratio(); inc_t, mst_t = (set(inc.split()), set(mst.split())); tok_score = len(inc_t & mst_t) / len(inc_t | mst_t) if inc_t and mst_t else 0.0; gen_score = max(seq_score, tok_score); cfg_score = _mac_configuration_match_score(inc, mst); return gen_score * 0.6 + cfg_score * 0.4 if cfg_score is not None else gen_score
def _structured_field_match_score(incoming_norm, master_norm):
    device = incoming_norm.get('device'); comparisons = [('Storage (GB)', 'storage'), ('Storage Type', 'storage_type'), ('Connectivity', 'connectivity'), ('Material', 'material'), ('Model_Year', 'model_year'), ('Case Size', 'case_size'), ('Charging Method', 'charging_method')]; score, weight = (0.0, 0.0)
...
def score_candidate(incoming_norm, master_norm):
    inc_m, mst_m = (incoming_norm.get('model_text'), master_norm.get('model_text')); m_score = _model_similarity(inc_m, mst_m) or 0.0; s_score = _structured_field_match_score(incoming_norm, master_norm) or 0.0; conflicts = ['model_identity_conflict'] if model_identity_conflict(inc_m, mst_m) else []
    ...
    return {'confidence': round(m_score * 0.7 + s_score * 0.3, 4), 'model_score': round(m_score, 4), 'structured_score': round(s_score, 4), 'model_number_match': compare_model_numbers(incoming_norm, master_norm), 'model_identity_conflict': 'model_identity_conflict' in conflicts, 'conflicts': conflicts}
...
def compare_model_numbers(incoming_norm, master_norm):
    inc = incoming_norm.get('model_numbers', frozenset()); mst = master_norm.get('model_numbers', frozenset())
    if not inc and (not mst):
        return 'both_absent'
    if not inc and mst:
        return 'master_only'
    if inc and (not mst):
...
def compare_master_relevant_fields(incoming_norm, master_norm):
    unknown_fields, different_fields = ([], []); device = incoming_norm.get('device')
    for field in DUPLICATE_FIELDS:
        ...
        if field == 'Retail Price':
            ...
            if _is_unknown(inc) or _is_unknown(mst):
                if _is_unknown(inc) and _is_unknown(mst):
...
def configuration_matches(incoming_norm, master_norm):
    ...
    if _is_unknown(inc_m) or _is_unknown(mst_m):
        ...
        if model_identity_conflict(inc_m, mst_m) or _model_similarity(inc_m, mst_m) < 0.8:
        ...
        for f_name, key in [('Storage (GB)', 'storage'), ('Storage Type', 'storage_type'), ('Connectivity', 'connectivity'), ('Material', 'material'), ('Model_Year', 'model_year'), ('Case Size', 'case_size'), ('Charging Method', 'charging_method')]:
            if is_field_applicable(incoming_norm.get('device'), f_name):
                inc, mst = (incoming_norm.get(key), master_norm.get(key))
                if _is_unknown(inc) or _is_unknown(mst):
                    unknown_fields.append(f_name)
                elif inc != mst:
                    different_fields.append(f_name)
...
def classify_incoming_row(incoming_row_canonical, incoming_norm, master_df, master_norm_by_index, master_device_by_index, master_provider_by_index, mapped_canonical_fields=None):
    reasons, warnings, inherited_fields = ([], [], []); mapped_canonical_fields = mapped_canonical_fields or set(); validation_errors = validate_row_data(incoming_row_canonical)
    ...
    best_candidate = choose_reference_candidate(candidates, incoming_norm.get('provider')); multi_match = len(candidates) > 1; retail_price_was_inherited = False; inc_rp, mst_rp = (incoming_norm.get('retail_price'), best_candidate['norm'].get('retail_price'))
    if _is_unknown(inc_rp) and (not _is_unknown(mst_rp)):
        incoming_norm['retail_price'] = mst_rp; incoming_row_canonical['Retail Price'] = mst_rp; inherited_fields.append('Retail Price'); retail_price_was_inherited = True
    ...
    if 'Storage Type' not in mapped_canonical_fields and _is_unknown(inc_st) and (not _is_unknown(mst_st)):
        incoming_norm['storage_type'] = mst_st; incoming_row_canonical['Storage Type'] = mst_st; inherited_fields.append('Storage Type')
    ...
    if 'Model Number' not in mapped_canonical_fields and (not inc_mns) and mst_mns:
        incoming_norm['model_numbers'] = mst_mns; incoming_row_canonical['Model Number'] = ', '.join(sorted(mst_mns)); inherited_fields.append('Model Number')
    incoming_norm['_inherited_fields'] = list(inherited_fields); comp = configuration_matches(incoming_norm, best_candidate['norm'])
    if comp['unknown_fields']:
        reasons.append('A Master counterpart exists, but one or more relevant fields are Unknown.'); return {'classification': 'conflict', 'conflict_type': 'needs_review', 'reasons': reasons, 'warnings': warnings, 'match': best_candidate, 'match_index': best_candidate['index'], 'multi_match': multi_match, 'unknown_fields': comp['unknown_fields'], 'different_fields': comp['different_fields'], 'model_numbers_to_append': [], 'model_number_update_required': False, 'model_number_flag': compare_model_numbers(incoming_norm, best_candidate['norm'])}
    if comp['different_fields']:
        reasons.append('The incoming row represents a valid row that is not an exact match for the selected Master configuration.'); return {'classification': 'new', 'conflict_type': None, 'reasons': reasons, 'warnings': warnings, 'match': best_candidate, 'match_index': best_candidate['index'], 'multi_match': multi_match, 'unknown_fields': [], 'different_fields': comp['different_fields'], 'model_numbers_to_append': [], 'model_number_update_required': False, 'model_number_flag': compare_model_numbers(incoming_norm, best_candidate['norm'])}
    full_comp = compare_master_relevant_fields(incoming_norm, best_candidate['norm']); mn_flag = compare_model_numbers(incoming_norm, best_candidate['norm'])
    if full_comp['unknown_fields']:
        reasons.append('The Master counterpart exists, but one or more Master-relevant fields are Unknown.'); return {'classification': 'conflict', 'conflict_type': 'needs_review', 'reasons': reasons, 'warnings': warnings, 'match': best_candidate, 'match_index': best_candidate['index'], 'multi_match': multi_match, 'unknown_fields': full_comp['unknown_fields'], 'different_fields': full_comp['different_fields'], 'model_numbers_to_append': [], 'model_number_update_required': False, 'model_number_flag': mn_flag}
    if mn_flag == 'append' and (not full_comp['unknown_fields']) and (not [f for f in full_comp['different_fields'] if f != 'Model Number']):
        existing_mns = set(best_candidate['norm'].get('model_numbers', frozenset())); inc_mns = set(incoming_norm.get('model_numbers', frozenset())); return {'classification': 'update', 'conflict_type': None, 'reasons': ['The Apple configuration already exists in Master, but the incoming Model Number is new and should be appended.'], 'warnings': warnings, 'match': best_candidate, 'match_index': best_candidate['index'], 'multi_match': multi_match, 'unknown_fields': [], 'different_fields': [], 'model_number_flag': 'append', 'model_numbers_to_append': sorted(inc_mns - existing_mns), 'model_number_update_required': True}
    if full_comp['exact']:
        reasons.append('The complete Master-relevant row already exists.'); return {'classification': 'conflict', 'conflict_type': 'duplicate', 'reasons': reasons, 'warnings': warnings, 'match': best_candidate, 'match_index': best_candidate['index'], 'multi_match': multi_match, 'unknown_fields': [], 'different_fields': [], 'model_numbers_to_append': [], 'model_number_update_required': False, 'model_number_flag': 'match'}
    diff_fields = full_comp['different_fields']
    if 'Provider' in diff_fields or 'Model Number' in diff_fields:
        reasons.append('The Apple configuration exists, but the Provider or Model Number differs.'); return {'classification': 'new', 'conflict_type': None, 'reasons': reasons, 'warnings': warnings, 'match': best_candidate, 'match_index': best_candidate['index'], 'multi_match': multi_match, 'unknown_fields': [], 'different_fields': diff_fields, 'model_numbers_to_append': [], 'model_number_update_required': False, 'model_number_flag': mn_flag}
    if 'Retail Price' in diff_fields:
        if retail_price_was_inherited:
            reasons.append('Retail Price was inherited from Master; only mutable value(s) differ, so update.'); return {'classification': 'update', 'conflict_type': None, 'reasons': reasons, 'warnings': warnings, 'match': best_candidate, 'match_index': best_candidate['index'], 'multi_match': multi_match, 'unknown_fields': [], 'different_fields': diff_fields, 'model_numbers_to_append': [], 'model_number_update_required': False, 'model_number_flag': mn_flag}
        reasons.append('Retail Price differs from the existing Master row; requires Admin review.'); return {'classification': 'conflict', 'conflict_type': 'needs_review', 'reasons': reasons, 'warnings': warnings, 'match': best_candidate, 'match_index': best_candidate['index'], 'multi_match': multi_match, 'unknown_fields': [], 'different_fields': diff_fields, 'model_numbers_to_append': [], 'model_number_update_required': False, 'model_number_flag': mn_flag}
    reasons.append('The Apple configuration, Provider, and Retail Price match; only mutable value(s) differ, update row.'); return {'classification': 'update', 'conflict_type': None, 'reasons': reasons, 'warnings': warnings, 'match': best_candidate, 'match_index': best_candidate['index'], 'multi_match': multi_match, 'unknown_fields': [], 'different_fields': diff_fields, 'model_numbers_to_append': [], 'model_number_update_required': False, 'model_number_flag': mn_flag}
def _build_strict_identity(row_norm):
    identity = []; device = row_norm.get('device')
    for field in DUPLICATE_FIELDS:
        if not is_field_applicable(device, field):
            continue
        if field == 'Model Number':
            identity.append(tuple(sorted(row_norm.get('model_numbers', frozenset())))); continue
        val = _field_value(row_norm, field); identity.append(UNKNOWN if _is_unknown(val) else round(val, 2) if isinstance(val, float) else val)
    return tuple(identity)
def detect_duplicates(canonical_df, mapped_fields=None):
    mapped_fields = set(mapped_fields or CANONICAL_FIELDS); groups, norm_by_idx = ({}, {})
    for idx, row in canonical_df.iterrows():
        norm = normalize_row_for_matching(row, mapped_fields); norm_by_idx[idx] = norm; groups.setdefault(_build_strict_identity(norm), []).append(idx)
...
def build_master_indexes(master_df):
    ...
    for idx, row in master_df.iterrows():
        norm = normalize_row_for_matching(row, set(CANONICAL_FIELDS)); m_norm[idx], m_dev[idx], m_prov[idx] = (norm, norm.get('device'), norm.get('provider'))
    return (m_norm, m_dev, m_prov)
def build_extra_column_info(raw_df, unmapped_columns):
    res = []
    for col in unmapped_columns:
        s = raw_df[col]; samples = [str(v) for v in s if not _is_blank(v)][:5]; res.append({'column': col, 'sample_values': samples, 'non_empty_count': int(s.notna().sum())})
    return res
...
def build_effective_record(pre_inference_row, normalized_row, effective_row, inherited_fields, matched_master_row=None):
    ...
    def _get(r, f):
        return r.get(f) if r is not None and hasattr(r, 'get') else None
    values, provenance = ({}, {})
    for field in CANONICAL_FIELDS:
        eff_val = _get(effective_row, field)
        if field in inherited_fields:
            prov = 'master_inherited'; master_disp = _get(matched_master_row, field)
            if not _is_blank(master_disp):
                eff_val = master_disp
...
def effective_record_to_row(effective_record):
    if not effective_record or not isinstance(effective_record.get('values'), dict):
        return {}
    return {k: v for k, v in effective_record['values'].items()}
def describe_classification(cls, c_type, reasons):
    defaults = {('new', None): 'This is a new Apple configuration and will be added to Master.', ('update', None): 'This matches an existing Master row; mutable values will be updated.', ('conflict', 'duplicate'): 'This row already exists in Master exactly as supplied.', ('conflict', 'needs_review'): 'A Master counterpart exists, but requires Admin confirmation.', ('conflict', 'invalid_data'): 'This row contains malformed data and must be fixed.'}; def_text = defaults.get((cls, c_type)); r_text = ' '.join(reasons) if reasons else None; return f'{def_text} {r_text}' if def_text and r_text else def_text or r_text or ''
def analyze_upload(raw_df, master_df, clean_fn, column_mapping_override=None):
    ...
    if master_df is None:
        master_df = pd.DataFrame(columns=CANONICAL_FIELDS)
    prop = propose_column_mapping(list(raw_df.columns))
    if column_mapping_override is not None:
        mapping = dict(column_mapping_override); unmapped = [c for c in raw_df.columns if c not in mapping]; missing_req = [f for f in ['Device', 'Standardized Model'] if f not in mapping.values()]
    else:
    ...
    if clean_fn is not None:
        ...
        if cleaned is not None:
            canonical_df = cleaned
    for field in CANONICAL_FIELDS:
        if field not in canonical_df.columns:
            canonical_df[field] = np.nan
    canonical_df = canonical_df[CANONICAL_FIELDS]; pre_inf_df = canonical_df.copy(); vocabulary = build_device_vocabulary(master_df); inferred_fields = set(); canonical_df['Device'] = canonical_df['Device'].astype(object); canonical_df['Sub-device'] = canonical_df['Sub-device'].astype(object)
    for idx in canonical_df.index:
        row = canonical_df.loc[idx]
        if _is_blank(row.get('Device')) or _is_blank(row.get('Sub-device')):
            row_text = build_row_search_text(row); inf_dev, inf_sub = infer_device_subdevice(row_text, vocabulary)
            if _is_blank(row.get('Device')) and inf_dev:
                canonical_df.at[idx, 'Device'] = inf_dev; inferred_fields.add('Device')
            if _is_blank(row.get('Sub-device')) and inf_sub:
                canonical_df.at[idx, 'Sub-device'] = inf_sub; inferred_fields.add('Sub-device')
    mn_maps = {}
    for idx in canonical_df.index:
    ...
    for idx in canonical_df.index:
        if _is_blank(canonical_df.at[idx, 'Model_Year']):
            ...
            if inf_yr is not None:
                canonical_df.at[idx, 'Model_Year'] = inf_yr; inferred_fields.add('Model_Year')
    display_maps = build_canonical_display_maps(master_df); canonical_df['Standardized Model'] = canonical_df['Standardized Model'].astype(object); canonical_df['Connectivity'] = canonical_df['Connectivity'].astype(object)
    for idx in canonical_df.index:
        for f, val in canonicalize_incoming_row_display_values(canonical_df.loc[idx], display_maps).items():
            canonical_df.at[idx, f] = val; inferred_fields.add(f)
    m_norm, m_dev, m_prov = build_master_indexes(master_df); mapped_fields = set(mapping.values()) | inferred_fields; within_file_dups = detect_duplicates(canonical_df, mapped_fields); extra_cols = build_extra_column_info(raw_df, unmapped); row_results, classifications = ([], [])
    for idx, row in canonical_df.iterrows():
        norm = normalize_row_for_matching(row, mapped_fields); wf_info = within_file_dups.get(idx, {}); is_wf_dup = wf_info.get('within_file_duplicate', False) and (not wf_info.get('is_first_occurrence', False)); norm_row = row.copy()
        if is_wf_dup:
            cls_res = {'classification': 'conflict', 'conflict_type': 'duplicate', 'reasons': ['Identical within-file duplicate.'], 'warnings': [], 'match': None, 'match_index': None, 'multi_match': False, 'unknown_fields': [], 'different_fields': [], 'model_number_flag': None, 'model_numbers_to_append': [], 'model_number_update_required': False}; row_inh = []
        else:
            cls_res = classify_incoming_row(incoming_row_canonical=row, incoming_norm=norm, master_df=master_df, master_norm_by_index=m_norm, master_device_by_index=m_dev, master_provider_by_index=m_prov, mapped_canonical_fields=mapped_fields); row_inh = norm.get('_inherited_fields', []); canonical_df.loc[idx, 'Retail Price'] = row.get('Retail Price')
        selected_m = cls_res.get('match'); m_master_row = master_df.loc[selected_m.get('index')] if selected_m and selected_m.get('index') in master_df.index else None; eff_rec = build_effective_record(pre_inference_row=pre_inf_df.loc[idx], normalized_row=norm_row, effective_row=row, inherited_fields=row_inh, matched_master_row=m_master_row); cls, c_type = (cls_res['classification'], cls_res.get('conflict_type')); classifications.append((cls, c_type)); row_results.append({'row_index': idx, 'classification': cls, 'conflict_type': c_type, 'reasons': cls_res['reasons'], 'warnings': cls_res['warnings'], 'match_index': cls_res['match_index'], 'multi_match': cls_res['multi_match'], 'unknown_fields': cls_res['unknown_fields'], 'different_fields': cls_res['different_fields'], 'model_number_flag': cls_res['model_number_flag'], 'model_numbers_to_append': cls_res.get('model_numbers_to_append', []), 'model_number_update_required': cls_res.get('model_number_update_required', False), 'within_file_duplicate': wf_info.get('within_file_duplicate', False), 'within_file_duplicate_group': wf_info.get('duplicate_group', []), 'incoming_row': _serialize_preview_row(norm_row), 'effective_record': eff_rec, 'what_this_means': describe_classification(cls, c_type, cls_res['reasons']), 'matched_master': _serialize_preview_row(m_master_row) if m_master_row is not None else None, 'match_confidence': selected_m.get('confidence') if selected_m else None, 'matched_master_row': selected_m.get('index') if selected_m else None})
    return {'mapping': mapping, 'unmapped_columns': unmapped, 'extra_columns': extra_cols, 'missing_required': missing_req, 'canonical_df': canonical_df, 'rows': row_results, 'summary': build_summary(classifications), 'master_norm_by_index': m_norm, 'master_device_by_index': m_dev, 'master_provider_by_index': m_prov, 'within_file_duplicates': within_file_dups}
...
def _append_model_numbers(master_df, match_index, model_numbers_to_append):
    if not model_numbers_to_append:
        return
    existing = master_df.at[match_index, 'Model Number']; is_mac = normalize_device_value(master_df.at[match_index, 'Device'] if 'Device' in master_df.columns else None) == 'Mac'; tokens = [t.strip() for t in _split_model_number_text(str(existing), is_mac) if t.strip()] if not _is_blank(existing) else []; norm_existing = normalize_model_numbers(existing, device=master_df.at[match_index, 'Device'] if 'Device' in master_df.columns else None)
    for t in model_numbers_to_append:
        if t not in norm_existing:
            tokens.append(t); norm_existing = norm_existing | {t}
    master_df.at[match_index, 'Model Number'] = ', '.join(tokens)
def apply_classified_rows(canonical_df, row_results, master_df):
    working_df = master_df.copy(); new_rows, outcomes = ([], [])
    for rr in row_results:
        idx, cls, c_type, m_idx = (rr['row_index'], rr['classification'], rr.get('conflict_type'), rr.get('match_index'))
        if cls == 'new':
            new_rows.append(canonical_df.loc[idx]); outcomes.append({'row_index': idx, 'action': APPLY_NEW, 'match_index': None, 'row_result': rr}); continue
        if cls == 'update':
            if m_idx is None or m_idx not in working_df.index:
                outcomes.append({'row_index': idx, 'action': APPLY_ERROR, 'match_index': m_idx, 'row_result': rr, 'error': 'Matched Master row missing.'}); continue
            ...
            outcomes.append({'row_index': idx, 'action': APPLY_UPDATE, 'match_index': m_idx, 'row_result': rr}); continue
        if cls == 'conflict' and c_type == 'duplicate':
            outcomes.append({'row_index': idx, 'action': APPLY_SKIPPED_DUPLICATE, 'match_index': m_idx, 'row_result': rr}); continue
